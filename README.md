...
3. **Comportamento de "Bloqueio Genuíno"**: os arquivos assumem, por padrão, que o fluxo **pausa de verdade** nesse caso (a opção mais conservadora das duas do prompt original). Se preferir "sempre segue com a opção conservadora e destaca, sem pausar", isso está concentrado principalmente em `orquestrador.md` (seção "Freios que continuam bloqueantes") e `templates/plano-tarefa.md` (seção "Exceção que nunca é resolvida sozinha") — ajuste os dois juntos para manter consistência.
