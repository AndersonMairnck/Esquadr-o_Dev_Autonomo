...
- **Requisito ou regra de negócio genuinamente ambígua o bastante para admitir leituras opostas**, sem qualquer sinal nos Requisitos que decida entre elas — critério: a decisão afeta como o negócio funciona ou como o usuário final experimenta o sistema (não só organização interna do código), e nenhuma fonte na ordem de precedência (plano → decisoes-autonomas.md → contrato → especificação técnica → documento de requisitos) resolve por analogia. Registre como "Bloqueio Genuíno" em `decisoes-autonomas.md` (seção "Bloqueios Genuínos"), com o que travou e as leituras possíveis, e **pare o fluxo de verdade, aguardando resposta do usuário** — mesma disciplina da "Regra de topo" no início deste documento. Nunca escolha uma leitura sozinho e siga adiante neste caso específico; isso é diferente de uma decisão Tier 2/ADR nova, que segue sem pausar.
